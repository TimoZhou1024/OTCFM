...
\begin{abstract}
This paper presents a structure-aware Flow Matching framework for multi-view clustering, designed to simultaneously perform missing information generation and alignment-free multi-view fusion in a unified and deterministic manner.
...
Although these methods have achieved promising performance in incomplete or unaligned multi-view clustering, they still suffer from several critical limitations. \textbf{First}, most existing methods treat incomplete and unaligned scenarios as separate problems, employing distinct pipelines for view imputation and correspondence alignment. This two-stage paradigm inevitably leads to error propagation between stages and suboptimal multi-view fusion. \textbf{Second}, recent diffusion-based IMVC methods \cite{zhangIncompleteMultiviewClustering2025a} achieve high-fidelity view generation but require hundreds of iterative denoising steps, resulting in $O(N \cdot r \cdot V \cdot T)$ forward passes for imputing missing views---a prohibitive latency for real-time MVC applications where rapid clustering updates are essential. \textbf{Third}, many UMVC methods \cite{huangPartiallyViewalignedClustering2020a,xinMultilevelReliableGuidance2025} require explicit correspondence supervision or annotated sample pairs, which is expensive or infeasible when cross-view correspondences are inherently unknown. 
...
Figure~\ref{fig:sensitivity_full} provides a comprehensive visualization of all nine parameters, showing the complete sensitivity landscape with optimal values marked.
