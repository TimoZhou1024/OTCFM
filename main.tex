...
\textbf{Execution Flow.} Figure~\ref{fig:overview} illustrates the complete pipeline: (1) Encode all available views into latent space; (2) Learn cluster centroids via soft clustering (Section~\ref{clustering}); (3) For aligned data, average latents to form consensus; for unaligned data, assign each view to nearest centroid; (4) Generate missing views via conditional flow matching; (5) Update model parameters using the unified objective.
...
\textbf{Self-Training via Auxiliary Target Distribution.} To encourage high-confidence cluster assignments, we follow the Deep Embedded Clustering (DEC) paradigm \cite{xieUnsupervisedDeepEmbedding2016} and define an auxiliary target distribution $P$ that sharpens the soft assignments:
\[
