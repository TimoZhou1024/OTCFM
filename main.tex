...
\textbf{Mitigating Mode Collapse.} The cold-start problem poses a risk of mode collapse in unaligned settings: poor initial centroids lead to poor conditioning, which in turn degrades latent representations. We address this through: (1) the warm-up phase that establishes meaningful representations before introducing centroid-conditioned flow; (2) periodic K-Means refinement that corrects drifting centroids; and (3) the GW loss that provides centroid-independent structural guidance. We empirically validate these design choices in our ablation studies.
