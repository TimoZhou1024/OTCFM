...
Although these methods have achieved promising performance in incomplete or unaligned multi-view clustering, they still suffer from several critical limitations. \textbf{First}, most existing methods treat incomplete and unaligned scenarios as separate problems, employing distinct pipelines for view imputation and correspondence alignment respectively. This two-stage paradigm inevitably leads to error propagation between stages and suboptimal multi-view fusion, as errors from the imputation stage directly compromise the subsequent alignment or clustering performance. \textbf{Second}, for incomplete multi-view scenarios, recent diffusion-based imputation methods \cite{ho2020ddpm,song2021scorebased,wang2025dcg} model data generation as a reverse denoising process governed by stochastic differential equations (SDEs). Specifically, these methods learn to iteratively denoise samples starting from Gaussian noise through a Markov chain $\mathbf{x}_T \to \mathbf{x}_{T-1} \to \cdots \to \mathbf{x}_0$, where each denoising step requires a neural network evaluation. Due to the stochastic nature of SDEs and the need to approximate continuous-time dynamics through discrete steps, generating a single sample typically requires $T = 100 \sim 1000$ sequential network forward passes to achieve acceptable quality. This iterative sampling paradigm introduces prohibitive computational latency (e.g., $\sim$10 seconds per sample with $T=1000$ \cite{song2021scorebased}), severely hindering deployment in latency-sensitive applications such as online recommendation systems or interactive data analysis tools. \textbf{Third}, for unaligned multi-view scenarios, many existing alignment methods \cite{huang2020partially,gong2025multi} require explicit correspondence supervision or annotated sample pairs for training, which is expensive or even infeasible to obtain in real-world scenarios where cross-view correspondences are inherently unknown. \textbf{Fourth}, even unsupervised alignment methods often rely on expensive combinatorial optimization procedures (e.g., solving the assignment problem via the Hungarian algorithm with $O(N^3)$ complexity per iteration), which severely limits their scalability to large-scale datasets and real-time applications.
